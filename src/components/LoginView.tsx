...
export default function LoginView({ lang, setLang, onLoginSuccess, onSwitchToDoctorLogin }: LoginViewProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      setError(lang === 'en' ? 'Please enter your name.' : 'សូមបញ្ចូលឈ្មោះរបស់អ្នក។');
      return;
    }

    if (pin.length < 4) {
      setError(lang === 'en' ? 'Access PIN must be at least 4 digits.' : 'លេខកូដ PIN ត្រូវតែមានយ៉ាងហោចណាស់ ៤ខ្ទង់។');
      return;
    }
  ...
  return (
    <div className={`min-h-screen bg-[#FEFAFB] flex flex-col justify-center items-center px-4 py-8 font-sans ${lang === 'kh' ? 'lang-kh' : ''}`} id="login-container">
        ...
        <div className="flex flex-col items-center text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs font-black text-[#FA6B90] tracking-widest uppercase mt-1">
              {lang === 'en' ? 'Digital Maternal Health Passport & Vault' : 'លិខិតឆ្លងដែនសុខភាពមាតា & ឃ្លាំងឯកសារ'}
            </p>
